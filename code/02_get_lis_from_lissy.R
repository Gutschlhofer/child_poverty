## This file should be run entirely in LISSY (choose programming language R).
#  Just copy-paste its contents and then copy-paste the output into a standard 
#  editor file and create a CSV.
...
# Deflator needed for abs poverty analysis
...
for(i in 1:length(identifiers)){
  ...
  # Australia and Germany have NAs in "nhhmem" or "nhhmem17"
  dat <- dat %>% filter(!is.na(hid),!is.na(dhi),!is.na(hpopwgt),!is.na(nhhmem),!is.na(nhhmem17))
  
  # Main setting ---------------------------------------------------------------
  # In order to run the robustness tests, comment out the lines of the respective 
  # test setting below and re-run the code in LISSY
  dat <- dat %>%
    dplyr::filter(dhi > 0)
  ...
  # The settings for the household equivalence scales used
  # The first two are the per-capita and the LIS squareroot equiv scale respectively
  # The rest adjusts either one parameter
  scales <- data.frame(
  ...
  # Start out_params and out_string string, used to create CSV once this file
  # is executed via LISSY.
  out_params <- paste("iso3","year", sep = ",")
  out_string <- paste(unique(dat$iso3), unique(dat$year), sep = ",")
  
  # For all scales defined above calculate relative and absolute child poverty
  # rates
  for(s in 1:nrow(scales)){
    ...
    rel_poverty <- sum(dat_child$sumhpopwgt17[dat_child$is_rel_pov])/sum(dat_child$sumhpopwgt17)
    
    if(suffix == "_lis"){
      # Make disaggregation (poverty levels if certain income component would not be there)
      ...
      rel_poverty_means <- sum(dat_child_disaggregated$sumhpopwgt17[dat_child_disaggregated$phi_means < dat_child_disaggregated$rel_povline])/sum(dat_child_disaggregated$sumhpopwgt17)
      rel_poverty_means_su <- sum(dat_child_disaggregated$sumhpopwgt17[dat_child_disaggregated$phi_means_su < dat_child_disaggregated$rel_povline])/sum(dat_child_disaggregated$sumhpopwgt17)
      rel_poverty_means_su_pt <- sum(dat_child_disaggregated$sumhpopwgt17[dat_child_disaggregated$phi_means_su_pt < dat_child_disaggregated$rel_povline])/sum(dat_child_disaggregated$sumhpopwgt17)
    ...
    dat_child <- dat_child %>%
      ...
      mutate(abs_pov_i = if_else(phi_ppp <= abs_povline, T, F))
    
    abs_poverty <- sum(dat_child$sumhpopwgt17[dat_child$abs_pov_i])/sum(dat_child$sumhpopwgt17)
